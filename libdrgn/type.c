			  ...
			  struct drgn_type **ret)
{
	...
	if (builder->template_builder.parameters.size > 0 && strchr(tag, '<') == NULL) {
		...
		for (size_t i = 0; i < builder->template_builder.parameters.size; i++) {
			struct drgn_type_template_parameter *param =
				&builder->template_builder.parameters.data[i];
			...
			// A template parameter can be either a value or a type
			// We try it as a value first, otherwise we'd get the type of the value
			struct drgn_object const *param_as_obj = NULL;
			if (err = drgn_template_parameter_object(param, &param_as_obj))
				goto error;

			if (param_as_obj != NULL) {
				if (err = drgn_format_object(param_as_obj, -1, DRGN_FORMAT_OBJECT_PRETTY, &param_name))
					goto error;
			} else {
				// It was not a value, we try again as a type
				struct drgn_qualified_type param_as_type = {};
				if (err = drgn_template_parameter_type(param, &param_as_type))
					goto error;

				if (err = drgn_format_type_name(param_as_type, &param_name))
					goto error;
			}

			if (!string_builder_append(&full_tag, param_name)) {
				free(param_name);
				goto enomem;
			}
